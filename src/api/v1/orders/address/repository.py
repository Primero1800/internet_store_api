...
class AddressesRepository:
    ...
    async def get_one_complex(
        ...
        orm_model: Address | None = result.unique().scalar_one_or_none()
    ...
    async def get_one(
        ...
        orm_model = await self.session.get(Address, user_id)
        if not orm_model:
            text_error = f"user_id={user_id}"
            raise CustomException(
                msg=f"{CLASS} with {text_error} not found"
    ...
    async def get_all(
        ...
        query_filter = filter_model.filter(select(Address))
        stmt_filtered = filter_model.sort(query_filter)

        stmt = stmt_filtered.order_by(Address.user_id)
